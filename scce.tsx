...
const ROLE_LABELS = {PESE:"PESE",DELEGADO_JE:"Delegado JE",DR_EVENTUAL:"DR Eventual",REGISTRO_SCCE:"Registro SCCE",JEFE_OPS:"Jefe Ops",ENCARGADO_GASTO:"Encargado Gasto",DIRECTOR_REGIONAL:"Director Regional",NIVEL_CENTRAL:"Nivel Central"};
const POLICIES = {
...
function canDo(action,user,caseObj=null){
...
// ─── CATÁLOGO ────────────────────────────────────────────────────────────────
let _localSeq=0;
function newLocalId(){return`LOC-${String(++_localSeq).padStart(4,"0")}`;}
function buildCatalogSeed(){
  const now=new Date().toISOString();
  const entries=[];
  Object.entries(CONFIG.regions).forEach(([rc,rd])=>{
    Object.entries(rd.communes).forEach(([cc,cd])=>{
      (cd.locals||[]).forEach(nombre=>{
        entries.push({idLocal:newLocalId(),nombre,region:rc,commune:cc,activoGlobal:true,activoEnEleccionActual:true,fechaCreacion:now,fechaDesactivacion:null,origenSeed:true});
      });
    });
  });
  return entries;
}
function getActiveLocals(catalog,region,commune){
  return catalog.filter(l=>l.region===region&&l.commune===commune&&l.activoGlobal&&l.activoEnEleccionActual);
}
function findActiveLocal(catalog,region,commune,nombre){
  return catalog.find(l=>l.region===region&&l.commune===commune&&l.nombre===nombre&&l.activoGlobal&&l.activoEnEleccionActual)||null;
}
// v1.9: detectar si el local snapshot de un caso diverge del catálogo actual
function checkLocalDivergence(caseObj,catalog){
...
function catalogSelfCheck(catalog){
  const v=[];
  catalog.forEach(l=>{
...
// ─── UTILIDADES ──────────────────────────────────────────────────────────────
const SLA_MINUTES={CRITICA:5,ALTA:15,MEDIA:60,BAJA:120};
const nowISO=()=>new Date().toISOString();
const tsISO=(m=0)=>new Date(Date.now()-m*60000).toISOString();
function fmtDate(iso){if(!iso)return"—";const d=new Date(iso);return d.toLocaleDateString("es-CL")+" "+d.toLocaleTimeString("es-CL",{hour:"2-digit",minute:"2-digit"});}
function timeDiff(a,b){if(!a||!b)return null;return Math.round((new Date(b)-new Date(a))/60000);}
function getElapsed(c){return Math.round((Date.now()-new Date(c.createdAt).getTime())/60000);}
function isSlaVencido(c){if(!c.createdAt||["Resuelto","Cerrado"].includes(c.status))return false;return getElapsed(c)>(SLA_MINUTES[c.criticality]||120);}
function getRecommendation(c){
  const el=getElapsed(c),sla=SLA_MINUTES[c.criticality]||120,br=el>sla;
...
function recColor(l){return{high:"#ef4444",medium:"#f97316",low:"#94a3b8"}[l]||"#94a3b8";}
function genId(region,commune,seq){return`${region}-${new Date().getFullYear()}-${commune}-${String(seq).padStart(3,"0")}`;}
function uuidSimple(){return"ev-"+Math.random().toString(36).slice(2,10)+"-"+Date.now().toString(36);}
function calcCriticality(ev){
  const vals=Object.values(ev),max=Math.max(...vals),sum=vals.reduce((a,b)=>a+b,0);
...
function critColor(c){return{CRITICA:"#ef4444",ALTA:"#f97316",MEDIA:"#eab308",BAJA:"#22c55e"}[c]||"#6b7280";}
function statusColor(s){return{"Nuevo":"#6366f1","Recepcionado por DR":"#a78bfa","En gestión":"#3b82f6","Escalado":"#ef4444","Mitigado":"#f97316","Resuelto":"#22c55e","Cerrado":"#6b7280"}[s]||"#6b7280";}
function calcCompleteness(c){
  ...
  if(c.timeline?.length>=3)f++;
  return Math.round(f/8*100);
}
function validateCaseSchema(c,catalog=[]){
  const e=[];
...
function chainHash(prev,ev){return simpleHash(prev+(ev.eventId||"")+(ev.type||"")+(ev.at||"")+(ev.actor||"")+(ev.caseId||"")+(ev.summary||""));}
function appendEvent(log,type,actor,role,caseId,summary){
  const prevHash=log.length?log[log.length-1].hash:"00000000";
  const ev={eventId:uuidSimple(),type,at:nowISO(),actor,role,caseId,summary,prevHash,hash:""};
  ev.hash=chainHash(prevHash,ev);
  return[...log,ev];
}
function buildSeedLog(events){
  let log=[];
  for(const e of events){
    const prevHash=log.length?log[log.length-1].hash:"00000000";
    const ev={eventId:uuidSimple(),...e,prevHash,hash:""};
    ev.hash=chainHash(prevHash,ev);
    log.push(ev);
  }
  return log;
...
// ─── SEED ────────────────────────────────────────────────────────────────────
function makeSeedCases(catalog){
  const snap=(r,co,n)=>{const l=catalog.find(x=>x.region===r&&x.commune===co&&x.nombre===n);return l?{idLocal:l.idLocal,nombre:l.nombre,region:r,commune:co,snapshotAt:tsISO(95)}:null;};
  return[
...
const S={
  ...
  nav:{background:"#1a1d2e",borderBottom:"1px solid #2d3748",padding:"8px 16px",display:"flex",alignItems:"center",gap:"6px",flexWrap:"wrap",minHeight:42},
  nBtn:(a)=>({background:a?"#3b82f6":"transparent",color:a?"#fff":"#94a3b8",border:"none",padding:"5px 10px",borderRadius:"4px",cursor:"pointer",fontSize:"12px"}),
  card:{background:"#1a1d2e",border:"1px solid #2d3748",borderRadius:"6px",padding:"12px"},
  badge:(color)=>({background:color+"22",color,border:"1px solid "+color+"44",borderRadius:"3px",padding:"2px 6px",fontSize:"11px",fontWeight:600}),
  ...
  inp:{background:"#0f1117",border:"1px solid #374151",borderRadius:"4px",padding:"6px 8px",color:"#e2e8f0",fontSize:"13px",width:"100%",boxSizing:"border-box"},
  lbl:{display:"block",marginBottom:"3px",color:"#94a3b8",fontSize:"11px",fontWeight:600,textTransform:"uppercase"},
  g2:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px"},
  g4:{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"8px"},
};

// ─── COMPONENTE PRINCIPAL ─────────────────────────────────────────────────────
export default function SCCE(){
  ...
  const [localCatalog,setLocalCatalog]=useState(()=>buildCatalogSeed());
  const [cases,setCases]=useState(()=>{const cat=buildCatalogSeed();return makeSeedCases(cat);});
  const [auditLog,setAuditLog]=useState(()=>makeSeedAudit());
  ...
  function startNewCase(){
    setNewCase({region:currentUser.region||activeRegion,commune:"",local:"",origin:{actor:currentUser.name,channel:"Teams",detectedAt:nowISO()},summary:"",detail:"",evidence:[]});
  ...
  function recepcionar(caseId){
    const c=cases.find(x=>x.id===caseId);
    if(!c||!canDo("recepcionar",currentUser,c))return notify("Sin permiso","error");
    setCases(prev=>prev.map(x=>x.id!==caseId?x:{...x,status:"Recepcionado por DR",updatedAt:nowISO(),timeline:[...x.timeline,{type:"RECEPCIONADO",at:nowISO(),actor:currentUser.id,note:`Recepcionado por ${currentUser.name}`}]}));
  ...
  function changeStatus(caseId,newStatus){
    ...
    const tlMap={Escalado:"ESCALATED",Mitigado:"MITIGATED",Resuelto:"RESOLVED",Cerrado:"CLOSED","En gestión":"IN_MANAGEMENT","Recepcionado por DR":"RECEPCIONADO"};
    const tsMap={Escalado:"escalatedAt",Mitigado:"mitigatedAt",Resuelto:"resolvedAt",Cerrado:"closedAt"};
    setCases(prev=>prev.map(x=>{
      ...
      return{...x,status:newStatus,...(tsMap[newStatus]?{[tsMap[newStatus]]:nowISO()}:{}),timeline:tl,updatedAt:nowISO()};
    }));
    setAuditLog(prev=>appendEvent(prev,"STATUS_CHANGED",currentUser.id,currentUser.role,caseId,`Estado → ${newStatus}`));
  }

  function validateBypass(caseId,decision,fundament){
  ...
  function requestReassessment(caseId,newEval,justification){
    const c=cases.find(x=>x.id===caseId);
  ...
  function addAction(caseId,action,responsible,result_){
    const c=cases.find(x=>x.id===caseId);
    if(!c||!canDo("update",currentUser,c))return notify("Sin permiso","error");
    setCases(prev=>prev.map(x=>{
      if(x.id!==caseId)return x;
  ...
  function addDecision(caseId,fundament){
    const c=cases.find(x=>x.id===caseId);
  ...
  function addComment(caseId,comment){
    setCases(prev=>prev.map(x=>x.id!==caseId?x:{...x,timeline:[...x.timeline,{type:"COMMENT",at:nowISO(),actor:currentUser.id,note:comment}],updatedAt:nowISO()}));
    setAuditLog(prev=>appendEvent(prev,"COMMENT_ADDED",currentUser.id,currentUser.role,caseId,comment.slice(0,80)));
  }

  function catalogAddLocal(nombre,region,commune,actor){
  ...
  function catalogDeactivate(idLocal,actor){
  ...
  function catalogReactivate(idLocal,actor){
    const e=localCatalog.find(l=>l.idLocal===idLocal);
  ...
  function catalogToggleEleccion(idLocal,actor){
    const e=localCatalog.find(l=>l.idLocal===idLocal);
    if(!e)return;
  ...
  function runSimulation(){
    ...
    setSimCases(sc);
    setSimReport({total:sc.length,critica:sc.filter(c=>c.criticality==="CRITICA").length,alta:sc.filter(c=>c.criticality==="ALTA").length,avgScore:(sc.reduce((s,c)=>s+c.criticalityScore,0)/sc.length).toFixed(1)});
    setSimSurvey({claridad:0,respaldo:0,submitted:false});
  ...
  function exportCSV(){
    ...
    cases.forEach(c=>rows.push([c.id,c.region,c.commune,c.local||"—",c.criticality,c.status,c.bypass?"SÍ":"No",c.bypassFlagged?"FLAGGED":"—",c.localSnapshot?.idLocal||"—",fmtDate(c.createdAt),c.completeness+"%"]));
  ...
  const metrics=useMemo(()=>({
    ...
    avgComp:visibleCases.length?Math.round(visibleCases.reduce((s,c)=>s+c.completeness,0)/visibleCases.length):0,
    flagged:visibleCases.filter(c=>c.bypassFlagged&&!c.bypassValidated).length,
  }),[visibleCases]);

  // ─── SUB-COMPONENTS ───────────────────────────────────────────────────────
  const TipWrap=({label,text,color="#94a3b8"})=>(
    <span style={{position:"relative",display:"inline-flex",alignItems:"center"}} className="tipWrap">
      <span style={{...S.badge(color),fontSize:"9px",cursor:"help"}}>{label} ℹ</span>
      <span className="tip" style={{position:"absolute",top:"125%",left:0,background:"#0b1220",color:"#e2e8f0",border:"1px solid #334155",borderRadius:"8px",padding:"10px 12px",width:"240px",fontSize:"11px",lineHeight:1.4,boxShadow:"0 10px 28px rgba(0,0,0,.45)",zIndex:999,display:"none",wordBreak:"break-word"}}>
        <div style={{fontWeight:800,marginBottom:4,color:"#fff"}}>{label}</div>
        <div style={{color:"#94a3b8"}}>{text}</div>
      </span>
    </span>
  );

  const SlaBadge=({c})=>isSlaVencido(c)?<span style={{...S.badge("#ef4444"),fontSize:"9px"}}>SLA VENCIDO</span>:null;

  const RecBadge=({c})=>{
    const rec=getRecommendation(c);
    return(
      <span style={{position:"relative",display:"inline-flex",alignItems:"center"}} className="tipWrap">
        <span style={{...S.badge(recColor(rec.level)),fontSize:"9px",cursor:"help"}}>{rec.icon} {rec.label}</span>
    ...
    return(
      <div style={{...S.card,cursor:"pointer",borderLeft:`3px solid ${critColor(c.criticality)}`,marginBottom:6,position:"relative"}} onClick={onClick}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
          ...
          <span style={{fontSize:"10px",color:"#475569"}}>{CONFIG.regions[c.region]?.communes[c.commune]?.name||c.commune}</span>
        </div>
        <div style={{display:"flex",gap:10,color:"#64748b",fontSize:"11px",flexWrap:"wrap",alignItems:"center"}}>
          <span>🕐 {fmtDate(c.createdAt)}</span>
          <span style={{color:c.completeness>=80?"#22c55e":c.completeness>=50?"#eab308":"#ef4444"}}>✓ {c.completeness}%</span>
      ...
      <div style={{...S.app,display:"flex",alignItems:"center",justifyContent:"center"}}>
        ...
        <div style={{...S.card,width:340}}>
          ...
          <div style={{marginTop:12,borderTop:"1px solid #2d3748",paddingTop:10}}>
            ...
            {USERS.map(u=><div key={u.id} style={{fontSize:"10px",color:"#64748b"}}><span style={{color:"#94a3b8",fontFamily:"monospace"}}>{u.username}</span> — <span style={{color:"#475569"}}>{ROLE_LABELS[u.role]}</span></div>)}
          </div>
        </div>
      </div>
    );
        ...
        <div style={{...S.card,background:"#1c1408",border:"1px solid #f9731644",marginBottom:10}}>
          ...
          {divergencias.map(x=>(
            <div key={x.caseId} style={{display:"flex",gap:6,alignItems:"center",marginBottom:3,fontSize:"11px",flexWrap:"wrap"}}>
              ...
              <button style={{...S.btn("dark"),fontSize:"9px",padding:"1px 6px"}} onClick={()=>{setSelectedCase(x.caseId);setView("detail");}}>Ver</button>
            </div>
          ))}
      ...
      {!crisisMode&&(
        <div style={{...S.card,marginBottom:8,display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
          <select style={{...S.inp,width:"180px",borderColor:"#3b82f6"}} value={activeRegion} onChange={e=>{setActiveRegion(e.target.value);setFilterState(p=>({...p,commune:""}));}}>
            {Object.entries(CONFIG.regions).map(([k,v])=><option key={k} value={k}>{v.name}</option>)}
          </select>
          ...
          <select style={{...S.inp,width:"150px"}} value={filterState.commune} onChange={e=>setFilterState(p=>({...p,commune:e.target.value}))}>
            ...
            {Object.entries(CONFIG.regions[activeRegion]?.communes||{}).map(([k,v])=><option key={k} value={k}>{v.name}</option>)}
          </select>
          ...
          {visibleCases.filter(c=>["CRITICA","ALTA"].includes(c.criticality)&&!["Resuelto","Cerrado"].includes(c.status)).map(c=>(
            <div key={c.id} style={{...S.card,borderLeft:`4px solid ${critColor(c.criticality)}`,marginBottom:6,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:6}}>
              ...
              <div style={{display:"flex",gap:4}}>
                <button style={S.btn("primary")} onClick={()=>{setSelectedCase(c.id);setView("detail");}}>Ver</button>
          ...
          {["Nuevo","Recepcionado por DR","En gestión","Escalado","Mitigado","Resuelto","Cerrado"].map(st=>{
            ...
            return(
              <div key={st} style={{marginBottom:10}}>
                ...
                {bucket.map(c=><CaseCard key={c.id} c={c} onClick={()=>{setSelectedCase(c.id);setView("detail");}}/>)}
              </div>
            );
          })}
  ...
  // ─── NEW CASE FORM ────────────────────────────────────────────────────────
  const NewCaseForm=()=>{
    const[lnc,setLnc]=useState(newCase||{});
    ...
    const rData=CONFIG.regions[lnc.region||"TRP"];
      ...
      <div>
          ...
          <div style={S.card}>
            ...
            <div style={{marginTop:10,textAlign:"right"}}>
              <button style={S.btn("primary")} onClick={()=>{
                ...
                setNewCase({...lnc});setBypassForm(lb);setStep(2);
              }}>Siguiente →</button>
            </div>
          </div>
        )}
        ...
        {step===2&&(
          <div style={S.card}>
            ...
            {varDefs.map(v=>(
              <div key={v.key} style={{...S.card,background:"#111827",marginBottom:6}}>
                ...
                <div style={{display:"flex",gap:6,alignItems:"center"}}>
                  {[0,1,2,3].map(n=>(
                    <button key={n} onClick={()=>setLe(p=>({...p,[v.key]:n}))} style={{padding:"6px 14px",borderRadius:4,border:"2px solid",cursor:"pointer",fontWeight:700,fontSize:"13px",background:le[v.key]===n?["#22c55e","#eab308","#f97316","#ef4444"][n]:"transparent",borderColor:["#22c55e44","#eab30844","#f9731644","#ef444444"][n],color:le[v.key]===n?"#fff":["#22c55e","#eab308","#f97316","#ef4444"][n]}}>{n}</button>
                  ))}
                  {le[v.key]===3&&<span style={{color:"#ef4444",fontWeight:700,fontSize:"11px"}}>⚠️ ESCALAR</span>}
                </div>
              </div>
            ))}
            ...
            <div style={{...S.card,background:"#111827",border:`2px solid ${critColor(er.criticality)}`,marginTop:8}}>
              <span style={S.badge(critColor(er.criticality))}>CRITICIDAD: {er.criticality}</span>
        ...
        {step===3&&(
          <div style={S.card}>
            ...
            <div style={{marginBottom:8}}>
              ...
              <textarea style={{...S.inp,height:70,resize:"vertical"}} placeholder="Describe el incidente..." value={newCase?.detail||""} onChange={e=>setNewCase(p=>({...p,detail:e.target.value}))}/>
            </div>
            <div>
              <label style={S.lbl}>Evidencia (Enter para agregar)</label>
              <input style={S.inp} placeholder="URL o descripción" onKeyDown={e=>{if(e.key==="Enter"&&e.target.value){setNewCase(p=>({...p,evidence:[...(p.evidence||[]),e.target.value]}));e.target.value="";}}}/>
        ...
        {step===4&&(
          <div style={S.card}>
            ...
            <div style={{...S.g2,marginBottom:8}}>
              <div><span style={{color:"#64748b"}}>Región:</span> {CONFIG.regions[newCase?.region]?.name}</div>
              <div><span style={{color:"#64748b"}}>Comuna:</span> {CONFIG.regions[newCase?.region]?.communes[newCase?.commune]?.name||newCase?.commune}</div>
              <div><span style={{color:"#64748b"}}>Canal:</span> {newCase?.origin?.channel}</div>
              <div><span style={{color:"#64748b"}}>Criticidad:</span> <span style={S.badge(critColor(calcCriticality(evalForm).criticality))}>{calcCriticality(evalForm).criticality}</span></div>
            </div>
  ...
  const CaseDetail=()=>{
    ...
    const canAssign=!isClosed&&canDo("assign",currentUser,c)&&(c.status==="Recepcionado por DR"||c.bypass||c.status==="En gestión"||c.status==="Escalado");
    const ca=auditLog.filter(e=>e.caseId===c.id);
    const assignee=USERS.find(u=>u.id===c.assignedTo);
    const div=checkLocalDivergence(c,localCatalog);
    const tlC={DETECTED:"#22c55e",REPORTED:"#3b82f6",FIRST_ACTION:"#f97316",ESCALATED:"#ef4444",RESOLVED:"#22c55e",CLOSED:"#6b7280",BYPASS:"#a78bfa",COMMENT:"#64748b",MITIGATED:"#eab308",RECEPCIONADO:"#a78bfa",REASSESSMENT:"#f97316",IN_MANAGEMENT:"#3b82f6",BYPASS_VALIDATED:"#22c55e",BYPASS_REVOKED:"#ef4444"};
    return(
      <div style={{position:"relative"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,flexWrap:"wrap",gap:6}}>
          ...
          <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
            ...
            {(canDo("update",currentUser,c)||canDo("close",currentUser,c))&&(
              <select style={{...S.inp,width:"auto"}} value={c.status} onChange={e=>changeStatus(c.id,e.target.value)}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          <div>
            <div style={{...S.card,marginBottom:8}}>
              ...
              <div style={S.g2}>
                <div><span style={{color:"#64748b"}}>Región:</span> {CONFIG.regions[c.region]?.name}</div>
                <div><span style={{color:"#64748b"}}>Comuna:</span> {CONFIG.regions[c.region]?.communes[c.commune]?.name||c.commune}</div>
                ...
                <div><span style={{color:"#64748b"}}>Complet.:</span> <span style={{color:c.completeness>=80?"#22c55e":c.completeness>=50?"#eab308":"#ef4444"}}>{c.completeness}%</span></div>
              </div>
            </div>
            ...
            <div style={{...S.card,marginBottom:8}}>
              ...
              {Object.entries({continuidad:"Continuidad",integridad:"Integridad jurídica",seguridad:"Seguridad",exposicion:"Exposición",capacidadLocal:"Capacidad local"}).map(([k,lbl])=>(
                <div key={k} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                  ...
                  <div style={{display:"flex",gap:3,alignItems:"center"}}>
                    {[0,1,2,3].map(n=><div key={n} style={{width:14,height:14,borderRadius:2,background:c.evaluation[k]>=n?["#22c55e","#eab308","#f97316","#ef4444"][n]:"#1e2535"}}/>)}
                    <span style={{marginLeft:4,color:["#22c55e","#eab308","#f97316","#ef4444"][c.evaluation[k]],fontWeight:700}}>{c.evaluation[k]}</span>
                  </div>
                </div>
              ))}
          ...
          <div>
            ...
            <div style={{...S.card,marginBottom:8}}>
              ...
              {c.actions.map(a=>{const u=USERS.find(u=>u.id===a.responsible);return(
            ...
            <div style={{...S.card,marginBottom:8}}>
              ...
              {c.decisions.map((d,i)=>{const u=USERS.find(u=>u.id===d.who);return(
              ...
              {canDo("close",currentUser,c)&&c.status!=="Cerrado"&&(
                <div style={{marginTop:8,padding:6,background:"#111827",borderRadius:4,fontSize:"10px"}}>
                  ...
                  {[[c.actions?.length,"Al menos 1 acción"],[c.decisions?.length,"Al menos 1 decisión"],[c.status==="Resuelto","Estado = Resuelto"],[!!c.closingMotivo,"Motivo guardado"],[!c.bypassFlagged||!!c.bypassValidated,"Bypass resuelto"]].map(([ok,lbl])=>(
                    <div key={lbl} style={{color:ok?"#22c55e":"#ef4444"}}>{ok?"✓":"✕"} {lbl}</div>
                  ))}
                </div>
              )}
            </div>
        ...
        <div style={{...S.card,marginTop:10}}>
          ...
          <div style={{maxHeight:140,overflowY:"auto"}}>
            {ca.map((e,i)=>{const u=USERS.find(u=>u.id===e.actor);const tc={CASE_CREATED:"#22c55e",BYPASS_USED:"#f97316",BYPASS_FLAGGED:"#ef4444",ESCALATED:"#ef4444",STATUS_CHANGED:"#eab308",ACTION_ADDED:"#94a3b8",EXPORT_DONE:"#6366f1",COMMENT_ADDED:"#64748b",REASSESSMENT:"#f97316",DECISION_ADDED:"#3b82f6",ASSIGNED:"#a78bfa"};
  ...
  const CatalogView=()=>{
    ...
    const rData=CONFIG.regions[catRegion];
    return(
      <div>
              ...
              return(
                <div key={l.idLocal} style={{display:"grid",gridTemplateColumns:"1fr 110px 80px 80px 70px 120px",gap:6,padding:"5px 0",borderBottom:"1px solid #1e2535",alignItems:"center",opacity:l.activoGlobal?1:0.5}}>
                  <div>
                    ...
                    <div style={{fontSize:"10px",color:"#475569"}}>{CONFIG.regions[l.region]?.communes[l.commune]?.name||l.commune}</div>
        ...
        <div style={{...S.card,marginTop:10}}>
          ...
          <div style={{maxHeight:130,overflowY:"auto"}}>
            {[...auditLog].filter(e=>["LOCAL_CREATED","LOCAL_DEACTIVATED","LOCAL_REACTIVATED","LOCAL_ELECTION_TOGGLED"].includes(e.type)).slice(-20).reverse().map((e,i)=>{
              ...
              const tc={LOCAL_CREATED:"#22c55e",LOCAL_DEACTIVATED:"#ef4444",LOCAL_REACTIVATED:"#f97316",LOCAL_ELECTION_TOGGLED:"#a78bfa"};
              return<div key={i} style={{display:"flex",gap:6,fontSize:"10px",padding:"3px 0",borderBottom:"1px solid #1e2535",flexWrap:"wrap"}}>
                <span style={{color:"#475569",width:108,flexShrink:0}}>{fmtDate(e.at)}</span>
                <span style={{color:tc[e.type],fontWeight:600,width:160,flexShrink:0}}>{e.type}</span>
  ...
  const Reports=()=>{
    ...
    const metricas=[
      ...
      ["Completitud promedio",cases.length?Math.round(cases.reduce((s,c)=>s+c.completeness,0)/cases.length):0, "%"],
      ...
      <div>
        ...
        <div style={{...S.g2,marginBottom:10}}>
          ...
          <div style={S.card}>
            ...
            {["CRITICA","ALTA","MEDIA","BAJA"].map(cr=>{const n=cases.filter(c=>c.criticality===cr).length;return(
        ...
        <div style={S.card}>
          ...
          <div style={{maxHeight:500,overflowY:"auto"}}>
            {[...auditLog].reverse().map((e,i)=>{
              ...
              const tc={CASE_CREATED:"#22c55e",BYPASS_USED:"#f97316",BYPASS_FLAGGED:"#ef4444",ESCALATED:"#ef4444",STATUS_CHANGED:"#eab308",ACTION_ADDED:"#94a3b8",EXPORT_DONE:"#6366f1",LOCAL_CREATED:"#22c55e",LOCAL_DEACTIVATED:"#ef4444",LOCAL_REACTIVATED:"#f97316",LOCAL_ELECTION_TOGGLED:"#a78bfa"};
        ...
        <div style={{...S.card,marginTop:10,border:"1px solid #6366f144"}}>
          ...
          {[{key:"claridad",label:"¿El sistema fue claro bajo presión?"},{key:"respaldo",label:"¿Los snapshots de local aportan confianza?"}].map(q=>(
            <div key={q.key} style={{marginBottom:8}}>
              ...
              <div style={{display:"flex",gap:4}}>
                {[1,2,3,4,5].map(n=>(
                  <button key={n} onClick={()=>setSimSurvey(p=>({...p,[q.key]:n}))} style={{padding:"4px 10px",borderRadius:3,border:"1px solid",cursor:"pointer",background:simSurvey[q.key]===n?"#3b82f6":"transparent",borderColor:simSurvey[q.key]===n?"#3b82f6":"#374151",color:simSurvey[q.key]===n?"#fff":"#64748b"}}>{n}</button>
                ))}
              </div>
            </div>
          ))}
